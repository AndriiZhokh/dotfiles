...
(defun my/monitor-dpi (&optional frame)
  "Return approximate DPI of the monitor FRAME is on."
  (let* ((attrs (frame-monitor-attributes frame))
         (geom (cdr (assq 'geometry attrs)))
         (mm (cdr (assq 'mm-size attrs)))
         (px-width (nth 2 geom))
         (mm-width (car mm)))
    (if (and mm-width (> mm-width 0))
        (/ (* px-width 25.4) mm-width)
      ;; WSL2 / headless: mm-size is bogus (0 or nil).
      ;; Guess DPI from pixel width: >=2560 => 2K+, >=1920 => FHD.
      (cond
       ((and px-width (>= px-width 3840)) 200.0)  ; 4K
       ((and px-width (>= px-width 2560)) 120.0)  ; 2K / QHD
       ((and px-width (>= px-width 1920)) 100.0)  ; 1080p
       (t 96.0)))))

(defun my/font-height-for-frame (&optional frame)
  "Compute a sensible font :height for FRAME's monitor."
  (let ((dpi (my/monitor-dpi frame)))
    (cond
     ((>= dpi 200) 160)   ; 4K / retina
     ((>= dpi 140) 130)   ; high-DPI laptop
     ((>= dpi 110) 130)   ; QHD / 2K
     (t             110)))) ; standard 1080p

(defun my/apply-font-for-frame (frame &rest _)
  (with-selected-frame frame
    (set-face-attribute 'default frame
                        :height (my/font-height-for-frame frame))))

;; Apply on creation and when moved/resized
(add-hook 'after-make-frame-functions #'my/apply-font-for-frame)
(add-hook 'move-frame-functions       #'my/apply-font-for-frame)
(add-hook 'window-size-change-functions
          (lambda (frame) (my/apply-font-for-frame frame)))

;; And once for the initial frame
(my/apply-font-for-frame (selected-frame))
#+end_src
...
(if (file-exists-p "~/.config/emacs/themes/compline-theme.el")
    (load-theme 'compline t)
  (message "Compline theme not found, using default theme"))
                                       ; load the theme
#+end_src
...
(with-eval-after-load 'projectile
  (when (eq system-type 'windows-nt)
    (let* ((rg-exe-path (shell-command-to-string "where rg.exe"))
           ;; 'where' can return multiple lines if multiple rg.exe exist; 
           ;; we take the first one and trim whitespace/newlines
           (clean-path (when (not (string-empty-p rg-exe-path))
                         (string-trim (car (split-string rg-exe-path "\n")))))
           ;; Get the directory containing the exe
           (rg-dir (when clean-path 
                     (file-name-directory clean-path))))

      (if (and rg-dir (file-directory-p rg-dir))
          (progn
            ;; Add to Emacs path
            (add-to-list 'exec-path rg-dir)
            ;; Add to System path for Projectile's subprocesses
            (setenv "PATH" (concat rg-dir ";" (getenv "PATH")))
            (message "Projectile: ripgrep found and loaded from %s" rg-dir))
        (message "Projectile: ripgrep (rg.exe) not found on system path.")))))
...
** VUI
Declarative UIs in Emacs with [[https://github.com/d12frosted/vui.el][vui.el]]

* PACKAGES TO TRY
** BUFFER TO PDF
Package to convert any emacs buffer to pdf file [[https://github.com/protesilaos/buffer-to-pdf][prot: buffer-to-pdf]]
** MEOW + GOD-MODE
Try to set up [[https://github.com/meow-edit/meow][meow]] and [[https://github.com/emacsorphanage/god-mode][god-mode]] instead of evil and evli-collection
[[https://joshblais.com/blog/meow-in-emacs/][Joshua Blais - Meow Made me Leave Evil-Mode]]
* HELPER FUNCTIONS
** DISPLAY INFO
