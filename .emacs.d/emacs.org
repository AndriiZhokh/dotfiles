...
(add-to-list 'custom-theme-load-path "~/.emacs.d/themes")  ; add custom theme directory
...
Also in original code from the github there was commented colors for org mode source blocks and I have fixed that

To use everforest theme I need to copy files `everforest-hard-dark-theme.el` and `everforest-hard-light-theme.el` into the root of config folder `.emacs.d`
** Compline Theme
#+begin_src emacs-lisp
...
** Projectile
To exclude file .projectile-cache.eld from git globally it need to be added to "$Env:USERPROFILE\.config\git\ignore" file, as well as .projectile file

if .confige\git\ignore file is not created it should be created manually and then need to run next command
`git config --global core.excludesfile "$Env:USERPROFILE\.config\git\ignore"` - for PowerShell for linux could be little bit different path
